...
```
This exercise reduced the dimensionality of the dataset to a much more manageable 56 columns. This is particularly important since this is not a linear problem. Instead I will be classifying with trees, a process that is prone to overfitting when a plethora of predictors are available. 
...
myTree <- randomForest(classe ~ ., method='rf', data=training)
print(myTree$finalModel)
```

The success rate on the training set is very high indeed. To validate the true effectiveness of this algorithm I will predict `classe` in every observation of the validation set (to test for overfitting), and then the imported `set_test` data. 
```{r, warning=FALSE}
...
A more detailed and precise error analyis is offered through a confusion matrix: 
```{r}
...
All five classes are represented here, which lends confidence to the validity of the model. 
